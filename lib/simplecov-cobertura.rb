    ...
    class CoberturaFormatter
      ...
      def set_branch_attributes(line, file_line, branches_by_line)
        ...
        if branch_info
          ...
          add_branch_conditions(line, branch_info[:branches])
        else
          line.attributes['branch'] = 'false'
        end
      end

      def add_branch_conditions(line, branches)
        line.add_element(conditions = REXML::Element.new('conditions'))

        branches.each_with_index do |(branch_type, hit_count), index|
          conditions.add_element(condition = REXML::Element.new('condition'))
          condition.attributes['number'] = index.to_s
          condition.attributes['type'] = branch_type
          condition.attributes['coverage'] = hit_count.to_i > 0 ? '100%' : '0%'
        end
      end
      ...
      def build_branches_by_line(file, branches_by_line)
        file.coverage_data.fetch("branches", {}).each do |condition, branches|
          ...
          info = branches_by_line[line] ||= { total: 0, covered: 0, branches: [] }
          branches.each do |branch_key, hit_count|
            branch_type = extract_branch_type(branch_key)
            info[:total] += 1
            info[:covered] += 1 if hit_count.to_i > 0
            info[:branches] << [branch_type, hit_count.to_i]
          end
        end
      ...
      def extract_branch_type(branch_key)
        return branch_key[0].to_s if branch_key.is_a?(Array)
        return branch_key unless branch_key.is_a?(String)

        branch_key.scan(/\w+/).first || 'jump'
      end
